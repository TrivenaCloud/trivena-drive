...
from trivena_drive.api.storage import storage_bar_data
...
from trivena_drive.utils.api import prettify_file
from trivena_drive.utils.files import FileManager, get_s3_key, get_s3_url
from trivena_drive.utils.users import mark_as_viewed
...
def upload_file(
    ...
    """
    checks = trivena.get_hooks("validate_drive_upload")
    for check in checks:
        res = trivena.call(check, file=trivena.request.files["file"], team=team, parent=parent, embed=embed)
        if res is not None and res is not True:
            trivena.throw(res or "This upload was cancelled by a validation check.", TypeError)

    home_folder = get_home_folder(team)
    parent = parent or home_folder["name"]

    if not user_has_permission(parent, "upload"):
        trivena.throw("Ask the folder owner for upload access.", trivena.PermissionError)

    team = trivena.db.get_value("File", parent, "team")
    if fullpath:
        parent = ensure_path(team, fullpath, parent)

    # Support both chunked and non-chunked uploads
    if trivena.form_dict.chunk_index:
        current_chunk = int(trivena.form_dict.chunk_index)
        total_chunks = int(trivena.form_dict.total_chunk_count)
        offset = int(trivena.form_dict.chunk_byte_offset)
    else:
        offset = 0
        current_chunk = 0
        total_chunks = 1

    file = trivena.request.files["file"]
    file_name = get_new_file_name(file.filename, parent)
    upload_session = trivena.form_dict.uuid
    ...
    if (storage_data["limit"] - storage_data["total_size"]) < file_size:
        trivena.throw("You're out of storage!", ValueError)
    ...
    trivena.publish_realtime("list-add", {"file": prettify_file(drive_file.as_dict())})
...
def get_thumbnail(entity_name: str):
    drive_file = trivena.get_cached_doc("File", entity_name)
    ...
    if not user_has_permission(drive_file, "read"):
        trivena.throw("No permission", trivena.PermissionError)
    ...
    response = Response(
        wrap_file(trivena.request.environ, thumbnail_data),
...
def create_folder(team: str, file_name: str, parent: str | None = None):
    ...
    parent_doc = trivena.get_doc("File", parent)
    ...
    drive_file = trivena.get_doc(
        {
            ...
            "file_url": link,
            "file_type": "Link",
            "file_modified": trivena.utils.now_datetime(),
            "folder": parent,
        }
    )
    drive_file.insert()
...
def create_auth_token(entity_name: str):
    ...
    token = trivena.get_doc(
        {
            "doctype": "Drive Token",
            "file": entity_name,
            "user": trivena.session.user,
            "expiry": trivena.utils.add_to_date(None, minutes=5),
        }
...
def get_file_content(entity_name: str, trigger_download: bool = False, token: str | None = None):
    ...
    if token:
        ...
        auth = trivena.db.get_value("Drive Token", token, ["file", "expiry"], as_dict=True)
        if not auth or auth.file != entity_name or trivena.utils.now_datetime() > auth.expiry:
            raise trivena.PermissionError("You do not have permission to view this file")
        trivena.delete_doc("Drive Token", token, ignore_permissions=True, force=True)
    elif not user_has_permission(entity_name, "read"):
        raise trivena.PermissionError("You do not have permission to view this file")

    file = trivena.get_value(
        "File",
        {"name": entity_name},
        [
            "file_name",
    ...
    if not file or file.file_type in FORBIDDEN_DOWNLOAD_TYPES or file.status != STATUS_ACTIVE:
        trivena.throw("Not found", trivena.DoesNotExistError)

    if file.file_type == "Document" or is_site_file(file):
        trivena.local.response["type"] = "redirect"
        trivena.local.response["location"] = file.file_url if is_site_file(file) else ("/drive/w/" + file.name)
        return
...
def get_file_internal(file, trigger_download=0):
    if not trigger_download and file.file_type == "Video" and trivena.request.headers.get("Range"):
    ...
    return send_file(
        ...
        environ=trivena.request.environ,
    )
...
def stream_file_content(entity_name: str):
    ...
    """
    range_header = trivena.request.headers.get("Range")
    if not range_header:
        return get_file_content(entity_name)
    entity = trivena.get_doc("File", entity_name)
    if not user_has_permission(entity, "read"):
        raise trivena.PermissionError("You do not have permission to view this file")

    if is_site_file(entity):
        trivena.local.response["type"] = "redirect"
        trivena.local.response["location"] = entity.file_url
        return
...
def _collect_download_files(entity_names):
    ...
    for name in entity_names:
        if not user_has_permission(name, "read"):
            raise trivena.PermissionError("You do not have permission to download this file")
        entity = trivena.get_value(
            "File",
            name,
...
def download_folder(entities: str):
    ...
    if isinstance(entities, str):
        entities = trivena.parse_json(entities)
    if not entities:
        trivena.throw("Nothing to download", ValueError)
    ...
    if not files:
        trivena.throw("No downloadable files found", trivena.NotFound)

    if len(entities) == 1:
        title = trivena.get_value("File", entities[0], "file_name")
        zip_name = f"{title}.zip"
    else:
        zip_name = f"Drive Download {trivena.utils.now()}.zip"
...
def set_favourite(entities: list | None = None, clear_all: bool = False):
    ...
    """
    if clear_all:
        return trivena.db.delete("Drive Favourite", {"user": trivena.session.user})

    if not isinstance(entities, list):
        trivena.throw(f"Expected list but got {type(entities)}", ValueError)

    for entity in entities:
        existing_doc = trivena.db.exists(
            {
        ...
        if not entity["is_favourite"] and existing_doc:
            trivena.delete_doc("Drive Favourite", existing_doc)
...
def remove_or_restore(entity_names: list[str] | str):
    ...
    if not isinstance(entity_names, list):
        trivena.throw(f"Expected list but got {type(entity_names)}", ValueError)
    manager = FileManager()

    def depth_zero_toggle_status(doc):
        if not user_has_permission(doc, "write"):
            raise trivena.PermissionError("You do not have permission to remove this file")
        ...
        doc.status = flag
        doc.file_modified = trivena.utils.now_datetime()
        # Only update parent folder size if parent exists (not root level)
        if doc.folder:
            folder_size = trivena.db.get_value("File", doc.folder, "file_size") or 0
    ...
    for entity in entity_names:
        depth_zero_toggle_status(trivena.get_doc("File", entity))


@trivena.whitelist()
def delete_entities(entity_names: list[str] | None = None, clear_all: bool = False):
    if clear_all:
        entity_names = trivena.db.get_list("File", {"status": STATUS_TRASHED, "owner": trivena.session.user}, pluck="name")
    elif isinstance(entity_names, str):
        entity_names = json.loads(entity_names)
    elif not isinstance(entity_names, list) or not entity_names:
        trivena.throw(f"Expected non-empty list but got {type(entity_names)}", ValueError)

    for entity in entity_names:
        trivena.get_doc("File", entity).permanent_delete()


@trivena.whitelist()
def rename(entity_name: str, new_title: str):
    drive_file = trivena.get_doc("File", entity_name)
...
def update_access(entity_name: str, method: str, **kwargs):
    ...
    kwargs.pop("cmd")
    if not drive_file:
        trivena.throw("Entity does not exist", ValueError)
    if method == "share":
...
def remove_recents(entity_names: list[str] | None = [], clear_all: bool = False):
    ...
    """
    if clear_all:
        return trivena.db.delete("Drive Entity Log", {"user": trivena.session.user})
    ...
    for entity in entity_names:
        ...
        if existing_doc:
            trivena.delete_doc("Drive Entity Log", existing_doc)
...
def does_entity_exist(name: str | None = None, folder: str | None = None, team: str | None = None):
    ...
    result = trivena.db.exists("File", {"folder": folder, "file_name": name})
    return result
...
def move(entity_names: list[str], new_parent: str | None = None, team: str | None = None):
    ...
    if not entity_names or not isinstance(entity_names, list):
        trivena.throw(f"Expected a non-empty list but got {type(entity_names)}", ValueError)

    for entity in entity_names:
        doc = trivena.get_doc("File", entity)
        res = doc.move(new_parent, team)

    if not res["folder"]:
        # Drive Team's display field is `title` (not `file_name`).
        title, personal = trivena.db.get_value("Drive Team", res["team"], ["title", "personal"])
...
def search(query: str):
    ...
    try:
        ...
        trivena.log_error(trivena.get_traceback(), "Frappe Drive Search Error")
        return {"error": str(e)}


@trivena.whitelist(allow_guest=True)
def translate_old_name(old_name: str):
    return trivena.get_value("File", {"old_name": old_name}, "name")


@trivena.whitelist(allow_guest=True)
def get_entity_type(entity_name: str):
    if not user_has_permission(entity_name, "read"):
        trivena.throw("You do not have permission to view this file.", trivena.PermissionError)

    entity = trivena.db.get_value(
        "File",
...
def get_root_folder(team: str):
    if team not in get_teams():
        trivena.throw("You can't check the home folder of a team you don't belong to.", trivena.PermissionError)
...
def redirect_to_original(file_id: str):
    ...
    """
    file = trivena.get_cached_doc("File", file_id)
    if not user_has_permission(file_id, "read"):
        trivena.throw("You do not have permission to view this file.", trivena.PermissionError)
    if not file.content_doctype == ATTACHMENT_CONTENT_DOCTYPE:
        trivena.throw("This is not an attachment", ValueError)

    trivena.local.response["type"] = "redirect"
    trivena.local.response["location"] = "/drive/g/" + file.content_docname
...
def get_docs_attached_to(file_name: str):
    file = trivena.get_doc("File", file_name)
...
def get_upload_path(file_name):
    root_folder = trivena.get_single("Drive Disk Settings").root_folder or ""
    uploads_path = Path(trivena.get_site_path("private/files"), root_folder, ".uploads")
