...
from trivena_drive.utils.api import get_default_access
from .permissions import get_user_access, user_has_permission, get_teams

DriveUser = trivena.qb.DocType("User")
UserGroupMember = trivena.qb.DocType("User Group Member")
DriveFile = trivena.qb.DocType("File")
DrivePermission = trivena.qb.DocType("Drive Permission")
Team = trivena.qb.DocType("Drive Team")
TeamMember = trivena.qb.DocType("Drive Team Member")
DriveFavourite = trivena.qb.DocType("Drive Favourite")
Recents = trivena.qb.DocType("Drive Entity Log")
...
def _apply_shared_filter(query, shared_type):
    ...
    """
    user = trivena.session.user if trivena.session.user != "Guest" else ""
...
def _get_public_files(names):
    ...
    if not names:
        return set()
    query = (
        trivena.qb.from_(DrivePermission)
...
def _get_basic_query(search):
    ...
    query = trivena.qb.from_(DriveFile).where(DriveFile.status == STATUS_ACTIVE)
    if search:
...
def files(
    ...
    if not entity_name:
        if team:
            ...
            trivena.throw("You must provide a folder to query", ValueError)
    ...
    if not user_has_permission(entity, "read"):
        trivena.throw(
            ...
            trivena.exceptions.PermissionError,
        )

    if is_site_file(entity):
        # Framework folder (e.g. desk "Home"): list via trivena.get_list so the
        ...
        names = trivena.get_list("File", filters=ff_filters, pluck="name")
        return get_query_data(
            trivena.qb.from_(DriveFile).where(DriveFile.name.isin(names or [""])),
...
def trash(
    ...
    query = (
        ...
        .where(DriveFile.owner == trivena.session.user)
    )
    if search:
        query = query.where(DriveFile.file_name.like(f"%{search}%"))
...
def get_query_data(
    ...
    query = query.on((DriveFavourite.entity == DriveFile.name) & (DriveFavourite.user == trivena.session.user)).select(
    ...
    if recents_only:
        query = (
            ...
            .on((Recents.entity_name == DriveFile.name) & (Recents.user == trivena.session.user))
...
def get_attachments(doctype: str | None = None, docname: str | None = None):
    ...
    if doctype and docname:
        files = trivena.get_list(
            "File", filters={"attached_to_doctype": doctype, "attached_to_name": docname}, pluck="name"
        )
        query = trivena.qb.from_(DriveFile).where(DriveFile.name.isin(files))
        return get_query_data(query)

    titles = {}
    if doctype:
        names = trivena.get_list("File", filters={"attached_to_doctype": doctype}, fields=["attached_to_name"])
        doctypes_set = Counter(k["attached_to_name"] for k in names)
        # Show each document by its title field rather than its raw name (ID).
        title_field = trivena.get_meta(doctype).get_title_field()
        if title_field != "name":
            titles = dict(
                trivena.get_all(
                    doctype,
