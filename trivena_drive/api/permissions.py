...
from trivena_framework.utils import getdate
from trivena_framework.model.document import Document
from trivena_framework.core.doctype.file.file import has_permission as ff_has_permission
...
def get_user_access(entity: str | Document | trivena._dict, user: str = None, team: bool = False):
    """
    Return the user specific permissions for an entity. Toggle `team` to check team permission.
    """
    if isinstance(entity, str):
        entity = trivena.get_cached_doc("File", entity)
    ...
    if is_site_file(entity):
        ...
        doc = entity if isinstance(entity, Document) else trivena.get_cached_doc("File", entity.name)
    ...
    if not user:
        if team:
            ...
            user = trivena.session.user
    # if not team and user not in [trivena.session.user, "Guest"] and not is_admin(entity.team):
    #     trivena.throw("You cannot check permissions of other users", PermissionError)

    # Owners and team members of a file have access
    teams = get_teams(user)

    if trivena.db.get_value("Drive Team", entity.team, "public"):
        access["read"] = 1
...
def is_admin(team: str):
    ...
    return drive_team[trivena.session.user].access_level == 2
...
def get_teams(user: str = None, details: bool = False, exclude_personal: bool = True):
    ...
    if details:
        teams_info = {
            team: {**trivena.get_doc("Drive Team", team).as_dict(), "file": get_home_folder(team)["name"]}
            for team in teams
        }
        if exclude_personal:
...
def get_public_teams():
    return trivena.get_all("Drive Team", fields=["name", "title"], filters=[["public", "=", 1]])
...
def get_entity_with_permissions(entity_name: str):
    ...
    if not entity:
        # Mimic API v2 points
        trivena.local.response.errors = [
            {
                "type": "PageDoesNotExistError",
                "message": "We couldn't find what you're looking for.",
            }
        ]
        trivena.throw("We couldn't find what you're looking for.", trivena.PageDoesNotExistError)
    entity = entity[0]
    ...
    if not user_access.get("read"):
        ...
        trivena.throw("You don't have access to this file.", trivena.PermissionError)

    owner_info = trivena.db.get_value("User", entity.owner, ["user_image", "full_name"], as_dict=True) or {}
    breadcrumbs = {"breadcrumbs": get_valid_breadcrumbs(entity.name, user_access)}
    favourite = trivena.db.get_value(
        "Drive Favourite",
        {
            "entity": entity_name,
            "user": trivena.session.user,
    ...
    trivena.response["data"] = return_obj
    return return_obj
...
def get_shared_with_list(entity: str):
    ...
    :rtype: list[trivena._dict]
    """
    if not user_has_permission(entity, "share"):
        raise trivena.PermissionError("You do not have permission to check the shares.")

    permissions = trivena.db.get_all(
        "Drive Permission",
    ...
    owner = trivena.db.get_value("File", entity, "owner")
    permissions.insert(
        0,
        trivena.db.get_value("User", owner, ["user_image", "full_name", "name as user"], as_dict=True),
    )

    for p in permissions:
        user_info = trivena.db.get_value("User", p.user, ["user_image", "full_name", "email"], as_dict=True)
        if user_info:
            p.update(user_info)
    return permissions
