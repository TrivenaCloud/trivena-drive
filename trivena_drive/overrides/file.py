...
from trivena_framework.core.doctype.file.file import File as FrappeFile
from trivena_framework.core.doctype.file.utils import get_content_hash
from trivena_framework.utils import get_files_path, now

from trivena_drive.api.files import get_file_type
from trivena_drive.api.permissions import get_user_access, user_has_permission
from trivena_drive.api.product import invite_users
from trivena_drive.api.activity import create_new_activity_log
...
class File(FrappeFile):
    ...
    def validate(self):
        ...
        if not self.is_private:
            trivena.throw("Drive files must be private.", trivena.ValidationError)
    ...
    def autoname(self):
        ...
        self.name = getattr(self, "_name", None) or trivena.generate_hash(length=10)

    def after_insert(self):
        if is_site_file(self) or trivena.flags.get("mute_drive_activity_log"):
            return
        full_name = trivena.db.get_value("User", trivena.session.user, "full_name")
    ...
    def after_delete(self):
        ...
        if self.is_folder:
            for child_name in trivena.get_all("File", filters={"folder": self.name}, pluck="name"):
                trivena.delete_doc("File", child_name, ignore_permissions=True)

        trivena.db.delete("Drive Favourite", {"entity": self.name})
        trivena.db.delete("Drive Entity Log", {"entity_name": self.name})
        trivena.db.delete("Drive Permission", {"entity": self.name})
        trivena.db.delete("Drive Notification", {"notif_doctype_name": self.name})
        trivena.db.delete("Drive Entity Activity Log", {"entity": self.name})

        if (
            self.content_doctype
            and self.content_docname
            and self.content_doctype != ATTACHMENT_CONTENT_DOCTYPE
            and trivena.db.exists(self.content_doctype, self.content_docname)
        ):
            trivena.delete_doc(self.content_doctype, self.content_docname, ignore_permissions=True)
    ...
    def share(
        ...
        if not user_has_permission(self, "share"):
            trivena.throw("Not permitted to share", trivena.PermissionError)

        # Clean out existing general records
        if not user or team:
            self.unshare("$GENERAL")

        permission = trivena.db.get_value(
            "Drive Permission",
            {
                "entity": self.name,
                "user": user or "",
        ...
        if not permission:
            permission = trivena.new_doc("Drive Permission")
            permission.update(
                {
                    "user": user,
                    "team": team,
            ...
            permission = trivena.get_doc("Drive Permission", permission)

        # Create user
        if user and not trivena.db.exists("User", user):
    ...
    def unshare(self, user: str | None = None):
        if not user_has_permission(self, "share"):
            trivena.throw("Not permitted to unshare", trivena.PermissionError)

        absolute_path = generate_upward_path(self.name)
        for i in absolute_path:
            if i["owner"] == user:
                trivena.throw("User owns parent folder", trivena.PermissionError)

        if user == "$GENERAL":
            perm_names = trivena.db.get_list(
                "Drive Permission",
                {"entity": self.name},
            ...
            for perm_name in perm_names:
                trivena.delete_doc("Drive Permission", perm_name, ignore_permissions=True)
            ...
            perm_name = trivena.db.get_value(
                "Drive Permission",
                {
                    "user": user,
                    "entity": self.name,
    ...
    def move(self, new_parent=None, new_team=None):
        ...
        if new_team and not new_parent:
            ...
            new_team = trivena.db.get_value("File", new_parent, "team")
        ...
        if new_parent == self.name:
            ...
            trivena.throw("You don't have permission to move this file.", trivena.PermissionError)

        if not trivena.db.get_value("File", new_parent, "is_folder"):
            trivena.throw(
        ...
        if new_parent != self.folder:
            old_folder_name = trivena.db.get_value("File", self.folder, "file_name")
            new_folder_name = trivena.db.get_value("File", new_parent, "file_name")
            full_name = trivena.db.get_value("User", trivena.session.user, "full_name")
            create_new_activity_log(
                entity=self.name,
                activity_type="move",
        ...
        return trivena.get_value("File", new_parent, ["file_name", "team", "name", "folder"], as_dict=True)
    ...
    def rename(self, new_file_name: str):
        ...
        if not user_has_permission(self, "write"):
            trivena.throw("You cannot rename this file", trivena.PermissionError)

        if new_file_name == self.file_name:
            return self
        validate_filename(new_file_name, self.folder, self.file_type)

        full_name = trivena.db.get_value("User", {"name": trivena.session.user}, ["full_name"])
        ...
        if len(new_file_name) > 140:
            trivena.throw("Your file_name can't be more than 140 characters.")
    ...
    def permanent_delete(self):
        ...
        if not (write_access or parent_write_access):
            trivena.throw("Not permitted", trivena.PermissionError)
    ...
    def get_children(self):
        ...
        child_names = trivena.get_list(self.doctype, filters={"folder": self.name}, pluck="name")
        for name in child_names:
            yield trivena.get_doc(self.doctype, name)
...
def after_upload_file(doc):
    ...
    if trivena.form_dict.library_file_name:
        library_doc = trivena.get_doc("File", trivena.form_dict.library_file_name)
        ...
        if not is_site_file(doc):
            ...
            doc.content_docname = trivena.form_dict.library_file_name
        ...
        save_folder = Path(trivena.get_site_path()) / file_path.parent
