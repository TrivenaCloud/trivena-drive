...
from trivena_framework.utils import get_fullname


def execute():
    all_shares = trivena.db.get_list(
        "Drive DocShare",
        fields=[
            "share_name",
            "user_doctype",
...
def create_activity_log(share):
    log = trivena.new_doc("Drive Entity Activity Log")
...
def update_activity_log(log, share):
    file_name = trivena.db.get_value("Drive File", share.share_name, ["file_name"])
    ...
    if share.everyone:
        ...
        log.new_value = share.user_name
    log.save()
    trivena.db.set_value("Drive Entity Activity Log", log.name, "message", message)
    trivena.db.set_value("Drive Entity Activity Log", log.name, "owner", share.owner)
    trivena.db.set_value("Drive Entity Activity Log", log.name, "creation", share.creation)
