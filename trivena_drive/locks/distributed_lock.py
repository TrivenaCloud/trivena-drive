...
class DistributedLock(object):
    def __init__(self, path, exclusive, ttl=60):
        ...
        self.ttl = ttl
        self.key = trivena.cache().make_key(path)
        self.lock_id = f"{trivena.session.user}{trivena.utils.now_datetime().timestamp()}"
        self.acquired = False
    ...
    def _add(self, key, value, ttl):
        ...
        return trivena.cache().set(key, value, ex=ttl, nx=True)
    ...
    def _increment(self, key, ttl):
        ...
        with trivena.cache().pipeline() as pipe:
            try:
    ...
    def _decrement(self, key):
        ...
        try:
            if not trivena.cache().exists(key):
                return False
            trivena.cache().decr(key)
            return True
        except redis.ResponseError:
            return False
