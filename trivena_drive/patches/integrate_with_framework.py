...
from trivena_framework.custom.doctype.custom_field.custom_field import create_custom_fields
from trivena_drive.utils import MIME_LIST_MAP, PRESENTATION_CONTENT_DOCTYPE, STATUS_ACTIVE, STATUS_TRASHED, WRITER_CONTENT_DOCTYPE
from trivena_drive.utils.files import get_s3_url
...
def ensure_custom_fields():
    ...
    with open(trivena.get_app_path("trivena_drive", "fixtures", "custom_field.json")) as f:
        fields = json.load(f)
...
def execute(files=None):
    if not trivena.db.table_exists(LEGACY_DOCTYPE):
        return

    ensure_custom_fields()

    trivena.flags.mute_drive_activity_log = True
    try:
        root_files = files or trivena.get_all(LEGACY_DOCTYPE, filters={"parent_entity": ["is", "not set"]}, pluck="name")

        is_remote = trivena.get_single("Drive Disk Settings").enabled
        failures = []
        ...
        trivena.flags.mute_drive_activity_log = False
...
def get_legacy(name):
    ...
    return trivena.get_all(LEGACY_DOCTYPE, filters={"name": name}, fields=["*"])[0]
...
def migrate_folder(folder_name, is_remote=False, failures=None):
    ...
    for child in trivena.get_all(LEGACY_DOCTYPE, filters={"parent_entity": folder_name}, fields=["*"]):
...
def migrate_file(file, is_remote=False, failures=None):
    ...
    if trivena.db.exists("File", file.name):
        return

    ff_file = trivena.get_doc(
        {
            "doctype": "File",
            "_name": file.name,
    ...
    if file.doc and trivena.db.exists(WRITER_CONTENT_DOCTYPE, file.doc):
        ff_file.content_doctype = WRITER_CONTENT_DOCTYPE
        ff_file.content_docname = file.doc

    if file.mime_type == "frappe/slides" and trivena.db.exists(PRESENTATION_CONTENT_DOCTYPE, file.path):
        ff_file.content_doctype = PRESENTATION_CONTENT_DOCTYPE
        ff_file.content_docname = file.path

    if trivena.db.get_value(LEGACY_DOCTYPE, file.parent_entity, "doc"):
    ...
    try:
        ff_file.insert()
        trivena.db.set_value("File", ff_file.name, "creation", file.creation, update_modified=False)
        trivena.db.set_value("File", ff_file.name, "owner", file.owner, update_modified=False)
        trivena.db.set_value("File", ff_file.name, "modified", file.modified, update_modified=False)
