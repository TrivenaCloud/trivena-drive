...
const tabs = [
  { key: 'home', label: __('Home'), icon: trivena.utils.icon('home', 'sm') },
  { key: 'teams', label: __('Teams'), icon: trivena.utils.icon('users', 'sm') },
]
...
onMounted(async () => {
  const { message } = await trivena.call('drive.api.permissions.get_teams', { details: 1 })
  teams.value = [{ name: SITE_TEAM, title: __('Site') }, ...Object.values(message || {})]
  const dt = await trivena.call('drive.utils.get_default_team')
...
async function fetchPage(offset) {
  ...
  if (isSiteTeam.value) {
    if (q) {
      ...
      const r = await trivena.call('trivena.core.api.file.get_files_by_search_text', { text: q })
      return { items: r.message || [], more: false }
    }
    const r = await trivena.call('trivena.core.api.file.get_files_in_folder', {
  ...
  const r = await trivena.call('drive.api.list.files', {
    team: t,
...
const ICON_BASE = '/assets/trivena_drive/images/icons'
...
async function siteUpload(file) {
  ...
  const res = await fetch('/api/method/upload_file', {
    ...
    headers: {
      ...
      'X-Frappe-CSRF-Token': trivena.csrf_token,
    },
    body: form,
  })
...
async function driveUpload(file, t, parent) {
  ...
  form.append('uuid', trivena.utils.get_random(10))
  const res = await fetch('/api/method/drive.api.files.upload_file', {
    method: 'POST',
    headers: { 'X-Frappe-CSRF-Token': trivena.csrf_token },
    body: form,
  })
  const data = await res.json()
  if (!res.ok) {
