<template>
  <div v-if="node" class="tree-node" :class="{ opened: node.open }">
    ...
    <div v-if="node.file_url && trivena.utils.is_image_file(node.file_url)">
...
let linkIcon = trivena.utils.icon('external-link', 'sm')
let checkIcon = trivena.utils.icon('tick', 'sm', '', '', 'check-icon')
let icon = computed(() => {
  if (!props.node.is_folder) return

  if (props.node.open) return trivena.utils.icon('folder-open', 'sm')
  return trivena.utils.icon('folder-normal', 'sm')
})
